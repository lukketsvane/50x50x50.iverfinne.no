import { buildMesh, DETAIL } from "../lib/straum/surface.ts"
import { DEFAULT_PARAMS } from "../lib/straum/params.ts"
import { STRAUM } from "../lib/straum/engine.ts"
// «flate» er den freste kroppen, «lag» er stabelen slik han står på
// benken. Begge skal vera lukka: den fyrste av di massen vert rekna av
// henne, den andre av di kvar del skal kunne skrivast til STL for seg.
const which = process.argv[2] ?? "flate"
const m =
  which === "flate"
    ? buildMesh(DEFAULT_PARAMS, DETAIL.mid)
    : STRAUM.build(DEFAULT_PARAMS as never, "mid", "lag")
