 ...
 *
 * Ei ribbe er ikkje eit uttrykk ein kan skrive ned. Midja kan bite så djupt
 * at ho deler ribba i eit fotstykke og eit setestykke; bogen kan gå gjennom
 * botnen og gjera to bein av eitt; setegropa kan kappe toppen. Kva form ho
 * får er eit spørsmål om kva som bit kvar, og det er nett det ei
 * marsjerande rute svarar på utan å bli spurt.
 *
...
/**
 * Rutenettet kjem ferdig utrekna og ikkje som ein funksjon. Det er ikkje
 * pynt: feltet til ei ribbe er skiljeleg — det meste av det avheng anten
 * berre av kolonnen eller berre av rada — og eit oppslag i to talrekkjer er
 * to potensrekningar mindre per rute enn eit kall.
 */
export function contour(
  ...
  /** der kanten kryssar null, lineært mellom dei to hjørna */
  ...
  for (let j = 0; j < ny; j++) {
    for (let i = 0; i < nx; i++) {
      ...
      // B = nedkanten, R = høgrekanten, T = overkanten, L = venstrekanten
      const B = () => hPt(i, j)
      const T = () => hPt(i, j + 1)
      const L = () => vPt(i, j)
      const R = () => vPt(i + 1, j)
