 ...
 *
 * SNITTBREIDDA
 * Fresen har breidd. Køyrer senteret langs den nominelle konturen, et halve
 * snittet seg inn i delen på kvar side, og delen kjem ut ein heil
 * snittbreidd for lita. På RIBBE er det ikkje ein finesse: sporklaringa er
 * 0,3 mm per side, og eit ukompensert kutt gjer kvart spor 3 mm for vidt —
 * ti gonger klaringa, og då held ikkje leddet noko som helst.
 */
...
export function partsToDxf(nesting: Nesting, plyT: number, kerf = 3): string {
  ...
  head(out, nesting.sheetW, nesting.sheets.length * pitch)
  ...
  nesting.sheets.forEach((sheet, i) => {
    ...
    for (const q of sheet.placed) {
      ...
      const at = centre(r.outline)
      text(out, "GRAVER", at, 14, q.part.id)
    }
  })
 ...
 * mindre enn skuvet — men på 1,5 mm mot ein hjørneradius på 6 mm er
 * skilnaden under ein tidels millimeter, og han går rette vegen.
 */
